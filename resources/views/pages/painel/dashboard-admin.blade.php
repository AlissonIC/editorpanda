            ...
            @forelse($albunsRecentes as $album)
                <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                    <div>
                        ...
                        <div class="text-muted small">{{ $album->user?->nome }} · {{ $album->evento?->nome }}</div>
                    </div>
    ...
    <div class="col-lg-5">
        <div class="panda-card h-100">
            ...
            <div class="d-grid gap-2">
                <a href="{{ route('painel.usuarios.index') }}" class="btn btn-outline-secondary text-start">
                    <i class="bi bi-people me-2"></i> Gerenciar usuários <span class="badge bg-secondary float-end">{{ $totalUsuarios }}</span>
                </a>
                <a href="{{ route('painel.financeiro.index') }}" class="btn btn-outline-secondary text-start">
